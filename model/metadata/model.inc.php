...
class Model_metadata {
    ...
    function __construct(){
        ...
        if (!($this->metadataList = unserialize(MC::get($key)))) {
            if (MC::getResultCode() == Memcached::RES_NOTFOUND) {
                $json=file_get_contents(Config::get('approot').'/www/js/addreserves/submit_types.json');
              ...
              error_log('Unaccountable memcached error '.MC::getResultCode());
            }
        }
    }


    function exists ($processingType){
        return isset($this->metadataList[$processingType]);
    }


    function getAllMetadataLists (){
        return $this->metadataList;
    }

    function getAllMetadataListsWithFieldSpecs(){
        $key = 'metadataListWithRequiredFields';

        if (!($res = unserialize(MC::get($key)))) {
            if (MC::getResultCode() == Memcached::RES_NOTFOUND) {
                $json = file_get_contents(Config::get('approot') . '/www/js/addreserves/submit_types.json');
                $json = str_replace(array("\n", "\r"), "", $json);
                $json = preg_replace('/([{,]+)(\s*)([^"]+?)\s*:/', '$1"$3":', $json);
                ...
                MC::set($key, serialize($res), MC::getDuration('short'));
            } else {
                error_log('Unaccountable memcached error ' . MC::getResultCode());
            }
        }
    ...
    function getMetadata($processingType)
    {
    ...
    function getMetadataMap($engine = null, $processingType)
    {
        $processingType = $this->_exists($processingType);
        ...
        error_log("Model_metadata:: Cannot find an engine to derive a metadata map.");
        return false;
    }
